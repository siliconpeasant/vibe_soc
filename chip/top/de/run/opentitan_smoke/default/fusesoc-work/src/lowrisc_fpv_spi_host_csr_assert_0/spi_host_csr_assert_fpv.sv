...
`define REGWEN_PATH spi_host.u_reg

// Block: spi_host
module spi_host_csr_assert_fpv import tlul_pkg::*;
    import top_pkg::*;(
  input clk_i,
  input rst_ni,
...
`ifndef SYNTHESIS
    ...
    if (!rst_ni) begin
      ...
      if (h2d.a_valid && d2h.a_ready) begin
        if ((normalized_addr inside {[0:52]})
            || (normalized_addr inside {[36: (36+1*8)]})
            || (normalized_addr inside {[40: (40+1*8)]})
           ) begin
  ...
  `ASSERT(control_rd_A,
          (d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
           pend_trans[d_source_idx].addr == 6'h10) |->
          (d2h.d_error ||
           (d2h.d_data & 'he000ffff) ==
            (exp_vals[3] & 'he000ffff)))
  `ASSERT(configopts_rd_A,
          (d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
           pend_trans[d_source_idx].addr == 6'h18) |->
          (d2h.d_error ||
           (d2h.d_data & 'hefffffff) ==
            (exp_vals[4] & 'hefffffff)))
  `ASSERT(csid_rd_A,
          (d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
           pend_trans[d_source_idx].addr == 6'h1c) |->
          (d2h.d_error ||
           (d2h.d_data & 'hffffffff) ==
            (exp_vals[5] & 'hffffffff)))
  `ASSERT(error_enable_rd_A,
          (d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
           pend_trans[d_source_idx].addr == 6'h2c) |->
          (d2h.d_error ||
           (d2h.d_data & 'h1f) ==
            (exp_vals[7] & 'h1f)))
  `ASSERT(event_enable_rd_A,
          (d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
