...
`define REGWEN_PATH pwrmgr.u_reg
...
`ifndef SYNTHESIS
  ...
  assign regwen[2] = 1;
  assign regwen[3] = 1;
  assign regwen[4] = `REGWEN_PATH.wakeup_en_regwen_qs;
  assign regwen[5] = 1;
  assign regwen[6] = `REGWEN_PATH.reset_en_regwen_qs;
  assign regwen[7] = 1;
  assign regwen[8] = 1;
  ...
  always_ff @(negedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (h2d.a_valid && d2h.a_ready) begin
        if ((normalized_addr inside {[0:64]})
           ) begin
  ...
  `ASSERT(intr_enable_rd_A,
          ...
          (d2h.d_error ||
           (d2h.d_data & 'h1) ==
            (exp_vals[0] & 'h1)))
  `ASSERT(wakeup_en_regwen_rd_A,
          (d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
           pend_trans[d_source_idx].addr == 7'h1c) |->
          (d2h.d_error ||
           (d2h.d_data & 'h1) ==
            (exp_vals[3] & 'h1)))
  `ASSERT(wakeup_en_rd_A,
          (d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
           pend_trans[d_source_idx].addr == 7'h20) |->
          (d2h.d_error ||
           (d2h.d_data & 'h3f) ==
            (exp_vals[4] & 'h3f)))
  `ASSERT(reset_en_regwen_rd_A,
          (d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
           pend_trans[d_source_idx].addr == 7'h28) |->
          (d2h.d_error ||
           (d2h.d_data & 'h1) ==
            (exp_vals[5] & 'h1)))
  `ASSERT(reset_en_rd_A,
          (d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
           pend_trans[d_source_idx].addr == 7'h2c) |->
          (d2h.d_error ||
           (d2h.d_data & 'h3) ==
            (exp_vals[6] & 'h3)))
  `ASSERT(wake_info_capture_dis_rd_A,
          (d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
