...
interface prim_onehot_check_if #(
  ...
  class prim_onehot_check_if_proxy extends sec_cm_pkg::sec_cm_base_if_proxy;
    ...
    virtual task inject_fault();
      ...
      bit addr_width_eq_1 = (AddrWidth == 1);
      bit addr_check = AddrCheck;
      bit enable_check = EnableCheck;
      bit strict_check = StrictCheck;
      ...
      `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(onehot_fault_type,
                                         addr_width_eq_1 -> onehot_fault_type != OnehotFault;
                                         !addr_check -> onehot_fault_type != OnehotAddrFault;
                                         !enable_check -> onehot_fault_type != OnehotEnableFault;)
      ...
      case (onehot_fault_type)
        OnehotFault: begin
          success = std::randomize(en_force_value, oh_force_value, addr_force_value) with {
              ...
              addr_check -> oh_force_value[addr_force_value] == (|oh_force_value);
            };
        end
