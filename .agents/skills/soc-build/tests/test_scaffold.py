...
class ScaffoldTest(unittest.TestCase):
    def test_canonical_project_contract(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((root / "scripts/syn/dc_synth.tcl").is_file())
            self.assertTrue((root / "scripts/formal/formality_verify.tcl").is_file())
            self.assertTrue((root / "scripts/clp/clp_rtl_upf_check.tcl").is_file())
            ...
            self.assertIn("comp: $(FILELIST)", common)
            self.assertIn("sim:\n", common)
