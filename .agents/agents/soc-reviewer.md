...
2. Select review depth: `quick` is an explicit compact audit; `normal` is the
   `merge` delivery review; `strict` is the `signoff` delivery review. Routine
   `dev` iterations do not dispatch this role.
...
5. Apply the project-rule source gate: only results whose source path starts with `soc/review/rule_library/` may be classified as `Project Rule`. All other knowledge-base results are `Reference Evidence` only and cannot establish a project violation, determine `Blocker`/`Critical` severity, or justify a waiver unless an applicable project rule explicitly incorporates that source.
6. Treat a project-rule result as a placeholder when it contains only a title/heading or lacks a substantive requirement. A placeholder cannot establish a violation or severity and requires proposed status `Need Human Confirmation`.
7. If `soc-ai-kb` is unavailable or no applicable substantive project rule is found, record that fact. Any issue whose conclusion depends on the missing rule must use proposed status `Need Human Confirmation`; never invent a rule ID or authoritative conclusion. Direct code defects, failed checks, missing artifacts, and repository process violations may still be reported from local evidence.
8. Run `<project_root>/.agents/scripts/check_loop_state.py <workspace> --mode <review_mode>` when a workspace is available.
9. Inspect `git status --short`, the relevant diff, and untracked files.
10. Verify that state, artifacts, check results, and claimed EDA evidence agree with real files and registered MCP execution. Treat stale/missing logs, estimated timing, direct shell fallback, illegal roots, and missing RTL-repair invalidation as findings.
11. Review every waiver separately. A waiver needs a rule/instance, owner, rationale, bounded impact, evidence, approval, and expiry or review condition. Do not create or approve waivers.
12. Report the review outcome as `pass`, `needs-fix`, `needs-validation`, or `blocked`. Do not modify source, state, generated artifacts, or waivers, and do not run EDA tools.
...
Use structured lists and tables, not an essay. For `quick`, emit only `Review
Summary`, actionable `Issue List`, and `Next Actions`; omit empty fields and
domains. For `normal` and `strict`, emit these sections in order:
...
- Rule authority (`Project Rule`, `Reference Evidence`, or `Local Evidence`)
- File path and line
- Associated report
- Description
- Evidence snippet
