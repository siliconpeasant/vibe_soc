...
- `orfs_dir`: OpenROAD-flow-scripts `flow/` directory, required only for explicit `backend=local`
- `platform`: ORFS platform, default `nangate45`
- `backend`: default `auto`, which uses Docker/Podman; local execution requires explicit `backend=local`
- `docker_image`: default `openroad/orfs:latest`
...
4. Call `soc-openroad.soc_openroad_run` for the requested stage with `backend=auto` unless the task explicitly requests local ORFS. No direct shell `make`, `openroad`, `yosys`, or ORFS fallback is allowed.
...
If Docker/Podman is unavailable, report that container execution is blocked. Do not start or continue a host-side OpenROAD source build unless the user explicitly changes the backend plan.
