...
- `orfs_dir`: OpenROAD-flow-scripts `flow/` directory, default `/project/xuanwu9000/user/silicon/OpenROAD-flow-scripts-master/flow`
...
5. Call `soc-openroad.soc_openroad_run` for the requested stage with default `backend=local`, `jobs=1`, and the default local ORFS directory. Prefer `config.local.mk` when it exists. No direct shell `make`, `openroad`, `yosys`, or ORFS fallback is allowed.
