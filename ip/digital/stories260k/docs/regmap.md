...
| `0x01C` | `GEN_CFG` | RW | `0x0004_0000` | Generation length minus one, softmax z-domain shift (default 2). |
...
| `0x10000-0x34FFF` | `WBUF` | RW | power-on zero | 148 KiB mixed-W4/W8 weight/scale/RoPE window (usage 150,208 B). |
...
Addresses not listed — including CSR offsets `0x038`, `0x040-0xFFF` and all of `0x01000-0x0FFFF`, `0x35000-0x3FFFF`, `0x5F000-0x5FFFF`, `0x61000-0x63FFF`, `0x66000-0xFFFFF` — return `mm_error=1` (`INVALID_ADDR`).
...
| `[20:17]` | `sm_shift` | RW | 2 | Softmax z-domain downshift: `z = clamp((s[t] − smax) >>> sm_shift, −128, 0)`, mapping the score-domain delta into the exp LUT's 1/16 input domain. Latched per run. The calibrated default for the shipped mixed-W4/W8 image is 2. |
...
|---|---:|---:|---|---:|---|
| `WBUF` | `0x10000` | 148 KiB | 4,736 × 32 B | 150,208 B (4,694 words) | Baseline tiles `+0x00000-0x201FF`, scales `+0x20200-0x222BF`, RoPE words 4374..4629, and WQ1 INT8 upper halves at words 4630..4693. |
