...
激活与查找表都在片上 293 KiB SRAM（0.286 MB）内，运行时无外部访存。
...
v1.2 把串行 SCORE/SOFTMAX/AV 改成 8-position tile 融合 attention。Design-B v1.7
在 QAT 上启用 **L1 QKV+WO + L2 WQ/WO + L3 WQ INT8**（WBUF 157 KiB / 5,024 字）、
`sm_shift=1`、decode 频次惩罚默认 `score-count*32`（CSR `DEC_CFG`）。预设
`DEC_PROFILE=golden|mid|long`（默认 golden 与 R4 bit-exact；`long`=64/1/12
抑 512 循环）。真实镜像 VCS 64-token 吞吐约 **12.8k tok/s @100 MHz**；首 64
token 与定点黄金模型 bit-exact。
...
| 权重 | L1 QKV+WO、L2 WQ+WO、L3 WQ 为 INT8，其余 INT4；8×8 tile + 每 64 元素 scale | MAC 内融合反量化；INT8 上下 4 行双读，不增加计算拍 |
...
## SRAM 预算（总计 293 KiB = 300,032 B = 0.286 MB）

| Buffer | 容量 | 用量 | 字宽 | 内容 |
|---|---:|---:|---|---|
| WBUF | 157 KiB | 160,448 B（5,014/5,024 字） | 32 B | 混合 W4/W8 权重 143,872 B + scales 8,384 B + RoPE 8,192 B；余 320 B |
...
地址窗口：CSR `0x00000-0x00FFF`，WBUF `0x10000-0x373FF`，KVBUF
`0x40000-0x5EFFF`，ACTBUF `0x60000-0x60FFF`，VECBUF `0x64000-0x65FFF`，
其余 `INVALID_ADDR`。`GEN_CFG[20:17]` 为 softmax z 域下移 `sm_shift`
（QAT 默认 1）。`DEC_CFG`（`0x038`）：`[7:0] rep_pen`（默认 32）、`[8] adapt_en`、
`[12:9] norep_win`。仿真：`DEC_PROFILE=golden|mid|long` 或 `make sim-long-anti`。
...
- 主机流程：加载 WBUF（tile 权重+scales+512-position RoPE）→ VECBUF（gains/结构化
  requant 0..34）→ 写 `TOKEN_IN`(BOS)、`GEN_CFG`（gen_len、sm_shift）、可选
  `DEC_CFG`（默认即可）→
...
- QAT 多长度 + 碎词评分（R5 选模；默认 golden decode）：

  ```bash
  make score-qat CKPT=assets/weights/stories260K_qat.bin
  # 或
  python3 dv/tests/score_qat_checkpoint.py --checkpoint ... --tokenizer assets/weights/tok512.bin \
    --teacher assets/weights/stories260K.bin --out-dir dv/sim/quality/score_qat
  ```

- 长故事 decode 预设：

  ```bash
  make sim-long-golden   # DEC_PROFILE=golden  32/0/0
  make sim-long-mid      # DEC_PROFILE=mid     48/0/0
  make sim-long-anti     # DEC_PROFILE=long    64/1/12
  ```
