...
| 输出 | `y[WIDTH-1:0]` |
| 功能 | `y = a | b` (位级按位或) |
| RTL 路径 | `ip/digital/soc_ip_common/de/rtl/std_cell/std_cell_or.v` |
...
- 综合命令:
  ...
  make syn RTL_TOP=std_cell_or
  ```
- 退出码: 0
- 实际 Yosys 脚本 (`de/syn/syn.ys`):
  ```
  read_verilog <filelist>
  hierarchy -check -top std_cell_or
  proc; flatten; opt; fsm; opt; memory; opt; techmap; opt
  write_verilog std_cell_or_netlist.v
  stat
  ```
...
| Latches | 0 |
| Memories | 0 |
| 估算 GE | ~1.25 GE (按典型 2 输入 OR 单元) |

## 时序结论

- 模块为**纯组合逻辑**,无寄存器,无时钟,无复位。
- WNS / TNS **不适用 (N/A)**,可视为 `+inf`。
- 关键路径: 输入 `a[i]` / `b[i]` → 1 级 `$_OR_` 门 → 输出 `y[i]`,延迟 < 1 ns (与工艺库 OR2 单元延迟一致)。
...
- 该模块为最小粒度标准单元,无优化空间。
- 后续如需替换为工艺库 cell,可在 yosys 中追加 `dfflibmap`/`abc -liberty <stdcells.lib>` 步骤,将 `$_OR_` 映射为具体的 `OR2_X1` 等单元。

## 产物路径

| 类型 | 路径 |
|------|----|
| 网表 | `ip/digital/soc_ip_common/de/syn/std_cell_or_netlist.v` |
| 综合日志 | `ip/digital/soc_ip_common/de/syn/synth.log` |
| 综合脚本 | `ip/digital/soc_ip_common/de/syn/syn.ys` |
| 本报告 | `ip/digital/soc_ip_common/de/syn/std_cell_or_synthesis_report.md` |
