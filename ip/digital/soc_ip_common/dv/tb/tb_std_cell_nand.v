...
module tb_std_cell_nand;
    ...
    initial begin
        $dumpfile("wave.vcd");
        $dumpvars(0, tb_std_cell_nand);

        $display("========================================");
        $display("  std_cell_nand Testbench (WIDTH=%0d)", WIDTH);
        $display("========================================");
        ...
        if (y !== {WIDTH{1'b0}}) begin
            $display("[FAIL] a=all1, b=all1, y=%h (expect 0)", y);
            errors = errors + 1;
        end else begin
            $display("[PASS] all-one inputs => y=0");
        end
