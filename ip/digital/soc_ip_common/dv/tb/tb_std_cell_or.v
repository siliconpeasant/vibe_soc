...
module tb_std_cell_or;
    ...
    initial begin
        $dumpfile("wave.vcd");
        $dumpvars(0, tb_std_cell_or);

        $display("========================================");
        $display("  std_cell_or Testbench (WIDTH=%0d)", WIDTH);
        $display("========================================");
        ...
        // Test 2: 全 0 / 全 1 边界
        a = {WIDTH{1'b0}}; b = {WIDTH{1'b0}}; #1;
        if (y !== {WIDTH{1'b0}}) begin
            $display("[FAIL] a=0, b=0, y=%h (expect 0)", y);
            errors = errors + 1;
        end else begin
            $display("[PASS] all-zero inputs => y=0");
        end

        a = {WIDTH{1'b0}}; b = {WIDTH{1'b1}}; #1;
        if (y !== {WIDTH{1'b1}}) begin
            $display("[FAIL] a=0, b=all1, y=%h (expect all1)", y);
            errors = errors + 1;
        end else begin
            $display("[PASS] zero a, all-one b => y=all1");
        end
        ...
        if (y !== {WIDTH{1'b1}}) begin
            $display("[FAIL] a=all1, b=0, y=%h (expect all1)", y);
            errors = errors + 1;
        end else begin
            $display("[PASS] all-one a, zero b => y=all1");
        end
