...
The verification target is the minimal INT8 inference-layer behavior: multi-output GEMV-style tile execution, signed INT32 bias, fixed-point multiplier/shift requantization, output zero point, optional activation clamp, signed INT8 saturation, and regression of the previous single-output dot-product profile.
...
- Scratchpad models for 64-byte activation, 64-byte weight, 64-byte output, and 16 signed INT32 bias entries.
...
| Descriptor activation out of range | Program activation base/count/stride so the final activation byte exceeds 63. | Command enters error before output store, `LAST_OUT_COUNT=0`, `ERR_CODE=DESC_ACT_RANGE`. |
| Descriptor weight out of range | Program legal activation but weight base/count/stride so the final weight byte exceeds 63. | Command enters error before output store, `LAST_OUT_COUNT=0`, `ERR_CODE=DESC_WGT_RANGE`. |
| Descriptor output out of range | Program output base/count/stride so an output byte exceeds 63. | Command enters error before output store, `LAST_OUT_COUNT=0`, `ERR_CODE=DESC_OUT_RANGE`. |
| Descriptor bias out of range | Program `BIAS_BASE + out_count_m1 > 15`. | Command enters error before output store, `LAST_OUT_COUNT=0`, `ERR_CODE=DESC_BIAS_RANGE`. |
...
- All `ERR_CODE` values.
- `k_count_m1` values for one group, multiple groups, and maximum in-range descriptor for the 64-byte scratchpads.
...
- Each scratchpad window: first word, middle word, and last aligned word.
- Bias entries 0, middle entries, and entry 15.
